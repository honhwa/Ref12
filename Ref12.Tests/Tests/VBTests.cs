...
namespace Ref12.Tests {
	...
	public class VBTests {
		...
		public async Task VBResolverMemberTests() {
			...
			var symbol = resolver.GetSymbolAt(fileName, textView.FindSpan("Environment.SetEnvironmentVariable(\"").End - 4);
			...
			symbol = resolver.GetSymbolAt(fileName, textView.FindSpan("EnvironmentVariableTarget.Process)").End - 1);
			Assert.AreEqual("mscorlib", symbol.AssemblyName);
			Assert.AreEqual("F:System.EnvironmentVariableTarget.Process", symbol.IndexId);

			symbol = resolver.GetSymbolAt(fileName, textView.FindSpan("AddHandler Microsoft.Win32.SystemEvents.PowerModeChanged").End);
			Assert.IsFalse(symbol.HasLocalSource);
			Assert.AreEqual("System", symbol.AssemblyName);
			Assert.AreEqual("E:Microsoft.Win32.SystemEvents.PowerModeChanged", symbol.IndexId);

			symbol = resolver.GetSymbolAt(fileName, textView.FindSpan("ex = New Exception").End);
		...
		public async Task VBResolverGenericsTests() {
			...
			var symbol = resolver.GetSymbolAt(fileName, textView.FindSpan("str.Aggregate").End);
			Assert.IsFalse(symbol.HasLocalSource);
